...
Pri pregledu sorodnih aplikacij sem prišla do ugotovitev, da slovenske aplikacije na trgu ni, saj je slovenski trg žal premajhen.

Ena izmed boljših aplikacij, ki jo trenutno uporabljam tudi sama, je aplikacija Track Yoga, kjer ima uporabnik možnost pregleda nad različnimi asanami, prikazanimi tudi z videoposnetki, prav tako pa ima na voljo različno dolge posnetke vodene prakse oz. možnost sledenja programu, npr. programu za izboljšanje fleksibilnosti, zmanjševanju stresa, meditacija itd,... V aplikaciji ima vsak uporabnik tudi svoj profil, kjer si lahko nastavi tedenske cilje, spremlja koliko programov in vodenih vadb je že opravil, sledi svojemu napredku z zbiranjem značk (angl. Badge) itd. 

Aplikacija je torej na nek način podobna moji, vendar pa nima možnosti delanja zapiskov in ne omogoča spremljanja lastnega napredka, kot je objavljanje slik, podajanje samoocene in interaktivno sledenje programu, kar bi uporabnika še bolj motiviralo.

Ena izmed aplikacij, ki so osredotočene bolj na učenje posameznika o asanah, pa je aplikacija Yoga 108. V aplikaciji uporavniku ponujajo pregled nad različnimi asanami, njihovimi izgovorjavami in angleškimi prevodi, vendar pa ne ponujajo podrobnega opisa asane, njenih prednosti in prikaza pravilne izvedbe z videoposnetkom. 
...
Diplomsko delo je razdeljeno na sedem večjih poglavij. 
Poglavje \textit{Uvod} je namenjeno predstavitvi sorodnih del, ter splošni strukturi razvoje diplomske naloge, zato je tudi razdeljeno na več podpoglavij. Sledi poglavje \textit{Joga} v katerem je predstavljena Joga, zgodovina joge in pomen asan. Poglavje \textit{Opis aplikacije in načrtovanje} je namenjeno predvsem predstavitvi glavnih funkcionalnosti, ki jih aplikacija ponuja. V naslednjem poglavu \textit{uporabljena tehnologija in orodja} je predstavljena arhitektura sistema in uporabljene tehnologije ter orodja, za razvoj aplikacije. V poglavju \textit{Predstavitev aplikacije} so s pomočjo zaslonskih posnetkov predstavljene funkcionalnosti aplikacije, v zaključnem poglavju pa je namenjeno sklepnim ugotovitvam in idejam za možni nadaljnji razvoj.
...
\section{Ideja in cilji diplome}
...
Glavne funkcionalnosti, ki jih ima uporabnik na voljo z aplikacijo AsanaMaster so sledeče:
 ...
 \begin{itemize}
  ...
  \item \textbf{Prijava}: prijava v aplikacijo z uporabniškim računom
  \item \textbf{Pozabljeno geslo}: resetiranje pozabljenega gesla s pošiljanjem novega gesla na elektronski naslov uporabnika
  \item \textbf{Ogled posnetkov}: ogled posnetkov v aplikaciji
  \item \textbf{Označevanje asane}: označevanje naučene asane
  \item \textbf{Ocenjevanje napredka}: ocenjevanje lastnega napredka
  \item \textbf{Nalaganje slik}: nalaganje slik napredka
  \item \textbf{Ogled slik}: ogled slik napredka
  \item \textbf{Kreiranje novega zapiska}: uporabnik lahko kreira zapisek
  \item \textbf{Brisanje zapiska}: uporabnik lahko željeni zapisek izbriše
  \item \textbf{Ogled profila}: ogled uporabniškega profila
\end{itemize}
